...
from mlprep.profiler import datasetProfile
...
def display_profile(df: pd.DataFrame):
    ...
    for column in profile.data_types.index:
        if column in profile.numerical_values: column_type="numerical"
        elif column in profile.categorical_values: column_type="Categorical"
        else:  column_type = str(profile.data_types[column])
